...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BCOM at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBCOM send
enum ZerocoinSpendStatus {
    ZBCOM_SPEND_OKAY = 0,                            // No error
    ZBCOM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBCOM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBCOM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBCOM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBCOM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBCOM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBCOM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBCOM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBCOM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBCOM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBCOM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBCOM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBCOM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBCOM_SPENT_USED_ZBCOM = 14,                      // Coin has already been spend
    ZBCOM_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZBitCommerceAutoBackups(bool fEnabled)
    {
