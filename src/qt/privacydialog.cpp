...
#include "zbitcommercecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBCOM ought to be enough for anybody." - Bill Gates, 2017
    ui->zBCOMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBCOMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBCOM->setEnabled(false);
        ui->pushButtonMintzBCOM->setToolTip(tr("zBCOM is currently disabled due to maintenance."));

        ui->pushButtonSpendzBCOM->setEnabled(false);
        ui->pushButtonSpendzBCOM->setToolTip(tr("zBCOM is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBCOMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBCOM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBCOM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBCOM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBCOM in ") + 
...
void PrivacyDialog::on_pushButtonSpendzBCOM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBCOM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBCOM
        sendzBCOM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBCOM
    sendzBCOM();
}

void PrivacyDialog::on_pushButtonZBitCommerceControl_clicked()
{
    ZBitCommerceControlDialog* zBitCommerceControl = new ZBitCommerceControlDialog(this);
    zBitCommerceControl->setModel(walletModel);
    zBitCommerceControl->exec();
}

void PrivacyDialog::setZBitCommerceControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBitCommerceSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBCOM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BitCommerce Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBCOMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBCOM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BCOM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBCOM</b>";
    ...
    // use mints from zBitCommerce selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBitCommerceControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitCommerceControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbitcommerce selector in case it was used
    ZBitCommerceControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBitCommerce Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BitCommerce, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBitCommerce Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBCOM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBCOM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBCOM "));
    ui->labelzBCOMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBCOMSyncStatus->setVisible(fShow);
}
