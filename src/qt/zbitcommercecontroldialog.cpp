...
#include "ui_zbitcommercecontroldialog.h"
...
std::list<std::string> ZBitCommerceControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitCommerceControlDialog::listMints;

ZBitCommerceControlDialog::ZBitCommerceControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitCommerceControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitCommerceControlDialog::~ZBitCommerceControlDialog()
{
    delete ui;
}

void ZBitCommerceControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitCommerceControlDialog::updateList()
{
...
void ZBitCommerceControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitCommerceControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitCommerce_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitCommerceControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitCommerceControlDialog::GetSelectedMints()
{
...
void ZBitCommerceControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
